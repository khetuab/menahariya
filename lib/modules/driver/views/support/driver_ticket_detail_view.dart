// lib/modules/driver/views/support/driver_ticket_detail_view.dart
...
class DriverTicketDetailView extends GetView<DriverSupportController> {
  const DriverTicketDetailView({Key? key}) : super(key: key);
            ...
            child: Obx(() {
              ...
              return ListView.builder(
                ...
                itemBuilder: (context, index) {
                  ...
                  final isDriverReply = reply.userRole == 'driver';
                  return _buildReplyBubble(context, reply, isDriverReply);
                },
              );
            }),
          ),
  ...
  Widget _buildReplyBubble(BuildContext context, ReplyModel reply, bool isDriverReply) {
    final theme = Theme.of(context);
    final isDark = theme.brightness == Brightness.dark;

    return Align(
      alignment: isDriverReply ? Alignment.centerRight : Alignment.centerLeft,
      child: Container(
        ...
        child: Column(
          crossAxisAlignment: isDriverReply ? CrossAxisAlignment.end : CrossAxisAlignment.start,
          children: [
            Padding(
              ...
              child: Text(
                isDriverReply ? 'You (Driver)' : (reply.user?.fullName ?? 'Admin'),
                style: theme.textTheme.bodySmall?.copyWith(
                  fontWeight: FontWeight.bold,
                  color: isDriverReply
                      ? Colors.blue
            ...
            Container(
              ...
              decoration: BoxDecoration(
                ...
                borderRadius: BorderRadius.only(
                  ...
                  bottomLeft: Radius.circular(isDriverReply ? AppDimens.radius12 : AppDimens.radius4),
                  bottomRight: Radius.circular(isDriverReply ? AppDimens.radius4 : AppDimens.radius12),
                ),
              ),
              child: Text(
                reply.message,
                style: theme.textTheme.bodyMedium?.copyWith(
                  color: isDriverReply ? Colors.white : (isDark ? AppColors.primaryGreenLight : AppColors.primaryGreen),
                ),
              ),
            ),
            Padding(
